...
Before any updates, invoke each of these skills with the Skill tool:

1. tk-rpie:maintaining-project-context
2. tk-rpie:writing-claude-md-files

If a skill is not yet present in the marketplace, STOP and report - do not proceed without the maintaining-project-context skill, which carries the format-detection and update protocol.
...
## Skill Dependency Note

This agent depends on the `maintaining-project-context` skill (and the related `writing-claude-md-files` skill), which originate in `ed3d-plugins/ed3d-extending-claude/skills/`. As of this port, those skills have NOT yet been ported into tk-rpie. Until they are, this agent will fail its Mandatory First Actions step. The skills must be ported next under the names:

- tk-rpie:maintaining-project-context
- tk-rpie:writing-claude-md-files

---
Provenance: ported from ed3d-plugins/ed3d-extending-claude (CC-BY-SA-4.0).
