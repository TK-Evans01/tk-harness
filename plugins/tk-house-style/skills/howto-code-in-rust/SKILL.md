...
- `tk-research:internet-researcher` for crate documentation, API behavior, and ecosystem conventions.
- `tk-research:remote-code-researcher` for examining external repositories for patterns and reference implementations.
...
- **Verify latest versions before adding or bumping.** Use `tk-research:internet-researcher` to look up the current version on crates.io. Memorized versions go stale quickly.
